...
static void *match_counter_source_create(obs_data_t *settings, obs_source_t *source)
{
    ...
    // ホットキーの設定
    context->win_hotkey = obs_hotkey_register_source(
        source, "match_counter_win", 
        obs_module_text("AddWin"),
        match_counter_win_hotkey, context);

    context->loss_hotkey = obs_hotkey_register_source(
        source, "match_counter_loss", 
        obs_module_text("AddLoss"),
        match_counter_loss_hotkey, context);

    context->reset_hotkey = obs_hotkey_register_source(
        source, "match_counter_reset", 
        obs_module_text("ResetCounter"),
        match_counter_reset_hotkey, context);
...
static obs_properties_t *match_counter_source_get_properties(void *data)
{
    ...
    obs_properties_add_button(props, "add_win", 
                             obs_module_text("AddWin"), 
                             match_counter_add_win_button);

    obs_properties_add_button(props, "add_loss", 
                             obs_module_text("AddLoss"), 
                             match_counter_add_loss_button);

    obs_properties_add_button(props, "subtract_win", 
                             obs_module_text("SubtractWin"), 
                             match_counter_subtract_win_button);

    obs_properties_add_button(props, "subtract_loss", 
                             obs_module_text("SubtractLoss"), 
                             match_counter_subtract_loss_button);

    obs_properties_add_button(props, "reset", 
                             obs_module_text("Reset"), 
                             match_counter_reset_button);
...
struct obs_source_info match_counter_source_info = {
    .id = "match_counter_source",
